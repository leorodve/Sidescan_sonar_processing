...
def upload():
    global filepath
    filepath = tkinter.filedialog.askopenfilenames(initialdir="/", title="Select A XTF File", filetypes=(("XTF files", "*.xtf"),("all files", "*.*")))
...
# Plot the lines in mosaic
def plot_data(window, Xutm, Yutm, histogram):
    ...
    if histogram == 0:
        ...
        maximum = 2**16/2        
    if histogram == 1:
        minimum = np.percentile((port_chan, stbd_chan), 0)
        maximum = np.percentile((port_chan, stbd_chan), 100)
    if histogram == 2:
        minimum = np.percentile((port_chan, stbd_chan), 1)
        maximum = np.percentile((port_chan, stbd_chan), 100)
    color_values = np.arange(minimum, maximum)    
    ax1.imshow(high_res_grid, cmap=my_colors, aspect='auto', vmin=np.percentile(color_values, 0), vmax=np.percentile(color_values, 100))
...
def plot_waterfall(window, port_data, stbd_data, settings):
    ...
    my_colors = sss.Color_palette()
    minimum = np.percentile((port_data, stbd_data), 1)
    maximum = np.percentile((port_data, stbd_data), 100)
    color_values = np.arange(minimum, maximum)
    ...
    if settings[7] == "AGC":
        ...
        port_data = data[:int(resampling/2), :]
        stbd_data = data[int(resampling/2):, :]
    ...
    ticks_x = np.arange(0, resampling+1, int(resampling/4))
    max_hor_range = sss.Get_max_range(filepath, file_number)
    increase = max_hor_range/4
    ticks_labels_x = np.arange(0, max_hor_range+1, increase)
...
def save_geotiff():
    ...
    ma.save_mosaic(high_res_grid, Xsmooth_utm, Ysmooth_utm, mosaic_resolution, filepath, minimum, maximum)
    print("Mosaic saved as geotiff in this folder: %s" %(os.getcwd()))
...
def fit_to_data():
    ...
    for widget in root.winfo_children():
        widget.destroy()
    scale_value = 1
    first_plot = plot_data(root, Xsmooth_utm, Ysmooth_utm, 1)
    first_plot.get_tk_widget().grid(row=1, column=0, columnspan=4)
    
    ttk.Button(root, text="Waterfall view", command=waterfall_window).grid(column=0, row=0)
    ttk.Button(root, text="Fit color scale to data", command=fit_to_data).grid(column=1, row=0)
    ttk.Button(root, text="Fit color scale to best", command=fit_to_best).grid(column=2, row=0)
    ttk.Button(root, text="Save mosaic as geotiff", command=save_geotiff).grid(column=3, row=0)
    
def fit_to_best():
    global scale_value
    for widget in root.winfo_children():
        widget.destroy()
    first_plot = plot_data(root, Xsmooth_utm, Ysmooth_utm, 2)
    first_plot.get_tk_widget().grid(row=1, column=0, columnspan=4)
    scale_value = 2
...
def display_data_window():
    global filepath, port_chan, new_port_chan, resampling, stbd_chan, new_stbd_chan, pings_per_file, Xsmooth_utm, Ysmooth_utm, high_res_grid, file_number, scale_value
    ...
    for file_number in range(len(filepath)):        
        if method_nav:
            ...
            '''
            THE NEXT FEW LINES ARE USING THE VALUE 75 FOR THE SLANT RANGE, THIS SHOULD BE USING AN AUTOMATED VALUE
            '''
            # Starboard and port utm coordinates
            ...
            # Matrix with port and starboard coordinates of ping
            ping_utmX = np.c_[stbd_utmX, port_utmX]
            ping_utmY = np.c_[stbd_utmY, port_utmY]
            ...
            # Coordenadas en latitud/longitud de starboard y port
            '''
            stbd_longitude1, stbd_latitude1 = sss.Lat_Long(stbd_utmX, stbd_utmY, True)
            port_longitude1, port_latitude1 = sss.Lat_Long(port_utmX, port_utmY, True)
            '''

        #matrix with ping measurements of port and starboard
        ...
        #matrix with all data measurements from port and starboard
    ...
    first_plot = plot_data(root, Xsmooth_utm, Ysmooth_utm, 0)
    first_plot.get_tk_widget().grid(row=1, column=0, columnspan=4)
