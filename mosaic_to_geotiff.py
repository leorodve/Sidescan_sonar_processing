...
def take_closest(myList, myNumber, length):
    ...
    """
    pos = [bisect_left(myList, myNumber[i]) for i in range(2*length)]
...
def grid_data(X, Y, X_utm, Y_utm, data, no_pings, resampling, resolution, file_number, file):
    mosaic = np.zeros((len(Y),len(X)))

    # position the data in the correct coordinate of the new grid
    file_name_with_ext = os.path.basename(file[file_number])
    file_name = os.path.splitext(file_name_with_ext)[0]
    print("Gridding data %s" %file_name)
    for a in tqdm(range(no_pings)):
        id_x = take_closest(X, X_utm[a,:,file_number], resampling)
        id_y = len(Y) - take_closest(Y, Y_utm[a,:,file_number], resampling)
        mosaic[id_y, id_x] = data[:,a,file_number]
...
def scalar_to_rgb(data, color_min, color_max):
...
def save_mosaic(mosaic, x_utm, y_utm, resolution, filepath, color_min, color_max):
    #save it first as png to create the rgb matrix
    img_rgb = scalar_to_rgb(mosaic, color_min, color_max)
